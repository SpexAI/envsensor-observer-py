...
def hci_le_read_local_supported_features(sock):
    bluez.hci_send_cmd(sock, OGF_LE_CTL,
                       OCF_LE_READ_LOCAL_SUPPORTED_FEATURES, b"")

def hci_le_read_remote_used_features(sock, handle):
    cmd_pkt = struct.pack("<H", handle)
    bluez.hci_send_cmd(sock, OGF_LE_CTL,
                       OCF_LE_READ_REMOTE_USED_FEATURES, cmd_pkt)
...
def hci_le_connect(sock, peer_bdaddr, interval=0x0004, window=0x0004,
    ...
    cmd_pkt = struct.pack("<HHBB", interval, window, initiator_filter,
                          peer_bdaddr_type) + package_bdaddr
    cmd_pkt += struct.pack("<BHHHHHH", own_bdaddr_type,
...
def hci_le_set_scan_parameters(sock, scan_type=LE_SCAN_ACTIVE,
                               interval=0x0010, window=0x0010,
...
def _handle_num_completed_packets(pkt):
    ...
    for _ in range(num_handles):
        ...
        result["handles"].append({"handle": handle,
                                  "num_completed_packets": completed})
        pkt = pkt[4:]
    return result
...
def _handle_command_status(pkt):
    ...
    return {"status": status, "number_of_commands": ncmd,
            "opcode": opcode, "opcode_group_field": ogf,
            "opcode_command_field": ocf}

def _handle_command_complete(pkt):
    ncmd, opcode = struct.unpack("<BH", pkt[:3])
    ogf, ocf = ogf_and_ocf_from_opcode(opcode)
    return {"number_of_commands": ncmd, "opcode": opcode,
            "opcode_group_field": ogf, "opcode_command_field": ocf,
...
def _handle_le_connection_complete(pkt):
    ...
    return {"status": status, "handle": handle, "role": role,
            "peer_bluetooth_address_type": bdaddr_type,
            "peer_device_address": addr, "interval": interval,
            "latency": latency, "supervision_timeout": timeout,
...
def _handle_le_advertising_report(pkt):
    ...
    for _ in range(num_reports):
        ...
        report["peer_bluetooth_address_type"] = struct.unpack("<B", pkt[offset+2:offset+3])[0]
        ...
        if data_len > 0:
            ...
            report["payload"] = packet_as_hex_string(report["payload_binary"],
                                                     flag_with_spacing=True,
                                                     flag_force_capitalize=True)
        rssi = struct.unpack("<b", pkt[offset+10+data_len:offset+10+data_len+1])[0]
        report["rssi"] = rssi
